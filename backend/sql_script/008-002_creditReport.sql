...
FROM sunlikeerp.overdueMonitor.dbo.paymentTerm a
	LEFT JOIN sunlikeerp.overdueMonitor.dbo.depositClientTotal b ON a.CUS_NO=b.CUS_NO
	LEFT JOIN (
		SELECT CUS_NO,SUM(AMTN_OUT) AS TOTAL_AMTN_OUT
		FROM sunlikeerp.overdueMonitor.dbo.outstanding
		GROUP BY CUS_NO) c ON a.CUS_NO=c.CUS_NO
	LEFT JOIN sunlikeerp.overdueMonitor.dbo.clientData d ON a.CUS_NO=d.CUS_NO

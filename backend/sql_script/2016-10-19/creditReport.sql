...
FROM UPGI_OverdueMonitor.dbo.paymentTerm a
	LEFT JOIN UPGI_OverdueMonitor.dbo.depositClientTotal b ON a.CUS_NO=b.CUS_NO
	LEFT JOIN (
		SELECT CUS_NO,SUM(AMTN_OUT) AS TOTAL_AMTN_OUT
		FROM UPGI_OverdueMonitor.dbo.outstanding
		GROUP BY CUS_NO) c ON a.CUS_NO=c.CUS_NO
	LEFT JOIN UPGI_OverdueMonitor.dbo.clientData d ON a.CUS_NO=d.CUS_NO
